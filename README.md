...
**Tested on:** Lambda Labs H100 SXM5 (80 GB), Ubuntu 22.04, Lambda Stack 22.04 image (`lambda-stack-22-04`)
...
**Note:** The base Lambda Stack 22.04 image ships with Python 3.10, but this script automatically provisions Python 3.12.11 via pyenv in a dedicated virtual environment to ensure consistency and compatibility.
...
The `eval_setup.sh` script is **idempotent, self-checking, and hard-fails on missing dependencies**. It:
...
- Installs **PyTorch 2.4.0** (CUDA 12.4) or **PyTorch 2.7.1** (CUDA 12.8+) with Flash Attention v2
...
- **human-eval-rust** >= 1.3.5 (from PyPI, GitHub fallback)
...
- **Fine-tuned Model:** `Superuser666-Sigil/Llama-3.1-8B-Instruct-Rust-QLora/checkpoint-9000`
  - HumanEval-Rust evaluation (no-policy mode)
  - HumanEval-Rust evaluation (policy-enforced mode)
...
This package guarantees a consistent, tested combination of ecosystem components:

| Component | Minimum Version | Purpose |
|-----------|----------------|---------|
| **sigil-pipeline** | >= 1.2.1 | Rust code dataset generation |
| **sigilderg-finetuner** | latest | QLoRA fine-tuning on Rust code |
| **human-eval-rust** | >= 1.3.5 | HumanEval-Rust evaluation harness |
