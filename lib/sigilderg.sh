...
# functionality to ensure correct installation. Requires human-eval-rust>=1.4.1.
#
...
install_sigilderg_components() {
    ...
    log_info "Installing human-eval-rust (requires >=1.4.1 for enhanced prompt format, result schema, compile rate tracking, main-free rate tracking, rustc preflight checks, and never-dropping completions)..."
    ...
    if "$PIP_CMD" install --force-reinstall --no-cache-dir "human-eval-rust>=1.4.1" 2>&1 | tee -a setup.log; then
