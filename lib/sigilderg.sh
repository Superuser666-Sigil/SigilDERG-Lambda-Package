...
# functionality to ensure correct installation. Requires human-eval-rust>=1.3.8.
#
...
install_sigilderg_components() {
    ...
    log_info "Installing human-eval-rust (requires >=1.3.8 for H100 optimizations, sandbox detection fix, main() function removal fix, Docker image pre-build, sample coverage fixes, safetensors fallback, TensorFlow weights support, and relaxed filtering: 4GB memory, 24 workers, 10s timeout, circular import fix, f-string syntax fix, sandbox auto-detect, main() regex fix)..."
    ...
    if "$PIP_CMD" install --force-reinstall --no-cache-dir "human-eval-rust>=1.3.8" 2>&1 | tee -a setup.log; then
