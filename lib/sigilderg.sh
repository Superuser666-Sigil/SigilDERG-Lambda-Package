...
# functionality to ensure correct installation. Requires human-eval-rust>=1.4.2.
#
...
install_sigilderg_components() {
    ...
    log_info "Installing human-eval-rust (requires >=1.4.2 for enhanced prompt format, result schema, compile rate tracking, main-free rate tracking, rustc preflight checks, never-dropping completions, and Docker/Finetuner parity)..."
    ...
    if "$PIP_CMD" install --force-reinstall --no-cache-dir "human-eval-rust>=1.4.2" 2>&1 | tee -a setup.log; then
