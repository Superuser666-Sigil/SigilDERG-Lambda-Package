...
5. **Installs the SigilDERG ecosystem and HumanEval-Rust**
   ...
   - Installs **`human-eval-rust`** from PyPI with a minimum version of **1.4.1** (required for enhanced prompt format, result schema, compile rate tracking, main-free rate tracking, rustc preflight checks, and never-dropping completions), and verifies the version.
