...
install_sigilderg_components() {
    ...
    if "$PIP_CMD" install --force-reinstall --no-cache-dir "human-eval-rust>=1.3.2" 2>&1 | tee -a setup.log; then
        ...
        if [[ "$GITHUB_VERSION" != "1.3.2" ]] && [[ "$GITHUB_VERSION" != "unknown" ]] && [[ -n "$GITHUB_VERSION" ]]; then
            log_warning "GitHub installation version $GITHUB_VERSION does not match expected 1.3.2"
            log_warning "This may indicate the GitHub main branch is not up to date. Consider using PyPI version 1.3.2+"
        fi
