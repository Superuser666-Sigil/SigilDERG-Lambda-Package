...
install_sigilderg_components() {
    ...
    log_info "Installing human-eval-rust (requires >=1.3.3 for H100 optimizations: 4GB memory, 24 workers, 10s timeout, circular import fix, f-string syntax fix)..."
    # Uninstall old version first to ensure clean install
    "$PIP_CMD" uninstall -y human-eval-rust 2>/dev/null || true
    # Force reinstall with version constraint to get H100 optimizations (1.3.3+)
    PYPI_INSTALL_SUCCESS=false
    if "$PIP_CMD" install --force-reinstall --no-cache-dir "human-eval-rust>=1.3.3" 2>&1 | tee -a setup.log; then
        ...
        if ! "$VENV_DIR/bin/python" -c "import human_eval; from human_eval.data import read_problems, get_human_eval_dataset" 2>&1 | tee -a setup.log; then
            ...
            if [[ "$INSTALLED_VERSION" == "unknown" ]] || [[ -z "$INSTALLED_VERSION" ]]; then
                if "$VENV_DIR/bin/python" -c "import human_eval" 2>/dev/null; then
                    ...
                    log_info "Package imports successfully, assuming version >=1.3.3 from PyPI"
                fi
            fi
            ...
            if [[ "$INSTALLED_VERSION" != "unknown" ]] && [[ -n "$INSTALLED_VERSION" ]]; then
                ...
                # Verify it's the correct version (allow 1.3.3+ format)
                if [[ "$INSTALLED_VERSION" != "1.3.3" ]] && [[ "$INSTALLED_VERSION" != "1.3.3+" ]] && [[ ! "$INSTALLED_VERSION" =~ ^1\.3\.[3-9] ]]; then
                    log_warning "Installed version $INSTALLED_VERSION may not have the latest fixes (expected >=1.3.3)"
                fi
            else
                ...
                log_info "Assuming version >=1.3.3 from PyPI installation (package installed successfully)"
    ...
    if [ "$PYPI_INSTALL_SUCCESS" = false ]; then
        ...
        if [[ "$GITHUB_VERSION" != "1.3.3" ]] && [[ "$GITHUB_VERSION" != "unknown" ]] && [[ -n "$GITHUB_VERSION" ]]; then
            log_warning "GitHub installation version $GITHUB_VERSION does not match expected 1.3.3"
            log_warning "This may indicate the GitHub main branch is not up to date. Consider using PyPI version 1.3.3+"
        fi
